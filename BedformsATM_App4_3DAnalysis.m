...
handles.projectfolder = [DocumentsPath '\Projects'];
...
aux = find(loaded_path=='\');
pos = strfind ( loaded_path, '\Projects\' );
pos = find(aux == pos) + 1;
handles.NameProject = loaded_path((aux(pos)+1):(aux(pos+1)-1));

[~,~,~] = mkdir ( [handles.projectfolder '\' handles.NameProject '\3DAnalysis'] );

handles.output_path = [handles.projectfolder '\' handles.NameProject '\3DAnalysis\'];
...
colormap gray;
surf(handles.FileBed.X,handles.FileBed.Y,H,'FaceColor','interp','EdgeColor','none','FaceLighting','gouraud');
hold on;
axis off;
