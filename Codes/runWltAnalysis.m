...
outputPath = [projectfolder '\Wavelet_Output'];
[~,~,~] = mkdir(outputPath);
printpath = [outputPath '\Figures'];
...
allPath = [outputPath '\Data_Output'];
[~,~,~] = mkdir (allPath);
...
for j = 1: numberOfFiles
    ...
    %% Save all variables
    save ([allPath '\' filename '.mat'],'j', 'signalDataX', 'signalData', 'sampleFreq',...
